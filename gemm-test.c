...
void do_AVX512_block(int n, int si, int sj, int sk, REAL *A, REAL *B, REAL *C)
{
  	int i,j,k;
	...
	for ( i = si; i < si+BLOCKSIZE; i+=8)
	{
    		for ( j = sj; j < sj+BLOCKSIZE; ++j)
  			...
  			__m512d c0 = _mm512_load_pd(C+i+j*n);   /* cij = C[i+j*n]] */
        		...
        		for( k = sk; k < sk+BLOCKSIZE; k++ )
 				c0 = _mm512_add_pd(c0,_mm512_mul_pd(_mm512_load_pd(A+i+k*n),
...
void do_AVX512_UNROLL_block(int n, int si, int sj, int sk, REAL *A, REAL *B, REAL *C)
{
  	int i,j,k,r;
...
#if 0
void do_block_OMP2 (int n, int si, int sj, int sk, REAL *A, REAL *B, REAL *C)
{
  	int i,j,k;
  	REAL cij;
...
void do_block_AVX_OMP2 (int n, int si, int sj, int sk, REAL *A, REAL *B, REAL *C)
{
  	int i,j,k;
  	REAL cij;
	#pragma omp parallel for private(j,k)
		...
		for ( i = si; i < si+BLOCKSIZE; i+=8)
		{
			for ( j = sj; j < sj+BLOCKSIZE; ++j)
				...
				for( k = sk; k < sk+BLOCKSIZE; k++ )
	 				c0 = _mm256_add_ps(c0,_mm256_mul_ps(_mm256_load_ps(A+i+k*n),
		...
		for ( i = si; i < si+BLOCKSIZE; i+=4)
		{
				...
				for( k = sk; k < sk+BLOCKSIZE; k++ )
	 				c0 = _mm256_add_pd(c0,_mm256_mul_pd(_mm256_load_pd(A+i+k*n),
...
void dgemm_AVX512_blocking (REAL *A, REAL *B, REAL *C, int n)
{
  	int sj,si,sk;	
...
void dgemm_AVX512_UNROLL_blocking (REAL *A, REAL *B, REAL *C, int n)
{
  	int sj,si,sk;	
...
void dgemm_AVX512_UNROLL_blocking_OMP (REAL *A, REAL *B, REAL *C, int n)
{
  	int sj,si,sk;	
...
/* AVX & AVX+OpenMP */
void dgemm_AVX2 (REAL *A, REAL *B, REAL *C, int n)	/*AVX2*/
{
	int i,j,k;
...
void dgemm_AVX512 (REAL *A, REAL *B, REAL *C, int n)
{
	int i,j,k;
...
void dgemm_AVX512_UNROLL (REAL *A, REAL *B, REAL *C, int n)
{
	int i,j,k,r;	
...
int check_mat (REAL *C, REAL *C_unopt, int N)     
{
	...
	for(n=0;n<N;n++)
	{
		...
		{
			max = C[n+N*m];
			if (C_unopt[n+N*m] > max)
				max = C_unopt[n+N*m];
			#if defined (FP_SINGLE)
				errrange = max/100000*0.5;
				if ((fabsf(C[n+N*m]-C_unopt[n+N*m])) > errrange )
				{
					printf("Error:   result is different in %d,%d  (%.2f, %.2f) delta %.2f > %.2f \n", n,m,C[n+N*m],C_unopt[n+N*m],fabsf(C[n+N*m]-C_unopt[n+N*m]),errrange);
					printf("errrange = %f\n",errrange);
				}
			#else
				errrange = max/10000000000000*0.5;
				if ((fabs(C[n+N*m]-C_unopt[n+N*m])) > errrange )
				{
					printf("Error:   result is different in %d,%d  (%.2f, %.2f) delta %.2f > %.2f \n", n,m,C[n+N*m],C_unopt[n+N*m],fabs(C[n+N*m]-C_unopt[n+N*m]),errrange);
					printf("errrange = %f\n",errrange);
				}
			#endif
		}
...
int main(int argc, char *argv[])
{
	...
	/*unoptimized*/
		...
		t = seconds() - t;
		printf("\n%f [s]  GFLOPS %f  |unoptimized| \n", t, (float)N*N*N*2/t/1000/1000/1000);
